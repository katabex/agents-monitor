...
Item {
  ...
  // Agents Monitor fork: refresh through the bundled runner instead of the
  // stock omarchy-agent-usage-update directly. The runner forwards to the
  // stock updater (claude, codex, fireworks) and also runs the bundled pi
  // collector, so all four providers refresh together.
  readonly property string updateBin: Qt.resolvedUrl("bin/agents-monitor-update").toString().replace(/^file:\/\//, "")
