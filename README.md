...
| **discovery** | — | the catalog-driven detection layer: parses Omarchy's agent catalog at runtime (`omarchy-menu.jsonc`, vendored fallback), applies `omarchy-default-agent`'s installed-semantics (on-demand mise stubs are NOT installs), probes stores, and writes MVP activity records (`activeDays`, no token claims) for used agents without a collector — Copilot today; the tab shows the terminal-prompt chevron mark until a real mark ships |
